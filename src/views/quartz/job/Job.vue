<template>
  <a-card :bordered="false" class="card-area">
    ...
    <div>
      <div class="operator">
        <a-button v-hasPermission="'admin:job:add'" ghost type="primary" @click="add">新增</a-button>
        <a-button v-hasPermission="'admin:job:delete'" @click="batchDelete">删除</a-button>
        <a-dropdown v-hasPermission="'admin:job:export'">
          <a-menu slot="overlay">
      ...
      <a-table ref="TableInfo"
        ...
        <template slot="operations" slot-scope="text, record">
          <a-icon v-hasPermission="'admin:job:update'" type="setting" theme="twoTone" twoToneColor="#4a9ff5" @click="edit(record)" title="修改"></a-icon>
          &nbsp;
          <a-dropdown v-hasAnyPermission="'admin:job:run','admin:job:pause','admin:job:resume'">
            <a class="ant-dropdown-link" href="#">
              <a-icon type="down-circle" style="font-size: 1.1rem"/>
            </a>
            <a-menu slot="overlay">
              <a-menu-item v-hasPermission="'admin:job:run'">
                <a href="javascript:void(0)" @click="runJob(record)">立即执行</a>
              </a-menu-item>
              <a-menu-item v-hasPermission="'admin:job:pause'" v-if="record.status === '0'">
                <a href="javascript:void(0)" @click="pauseJob(record)">暂停任务</a>
              </a-menu-item>
              <a-menu-item v-hasPermission="'admin:job:resume'" v-if="record.status === '1'">
                <a href="javascript:void(0)" @click="resumeJob(record)">恢复任务</a>
              </a-menu-item>
            </a-menu>
          </a-dropdown>
          <a-badge v-hasNoPermission="'admin:job:update','admin:job:run','admin:job:pause','admin:job:resume'" status="warning" text="无权限"></a-badge>
        </template>
      </a-table>
    </div>
    <!-- 新增任务 -->
...
export default {
  ...
  methods: {
    ...
    runJob (record) {
      ...
      this.$get('admin/job/run/' + jobId).then(() => {
    ...
    pauseJob (record) {
      let jobId = record.jobId
      this.$get('admin/job/pause/' + jobId).then(() => {
    ...
    resumeJob (record) {
      let jobId = record.jobId
      this.$get('admin/job/resume/' + jobId).then(() => {
    ...
    batchDelete () {
      ...
      this.$confirm({
        ...
        centered: true,
        onOk () {
          that.$delete('admin/job/' + that.selectedRowKeys.join(',')).then(() => {
