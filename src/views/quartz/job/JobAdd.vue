...
export default {
  ...
  methods: {
    ...
    handleSubmit () {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          this.$post('admin/job', formData).then(() => {
            this.reset()
            this.$emit('success')
          }).catch(() => {
            this.loading = false
    ...
    checkCron () {
      let cron = this.form.getFieldValue('cronExpression').trim()
      if (cron.length) {
        this.$get('admin/job/cron/check?cron=' + cron).then((r) => {
          if (!r.data) {
