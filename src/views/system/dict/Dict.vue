<template>
  <a-card :bordered="false" class="card-area">
    ...
    <div>
      <div class="operator">
        <a-button v-hasPermission="'admin:dict:add'" type="primary" ghost @click="add">新增</a-button>
        <a-button v-hasPermission="'admin:dict:delete'" @click="batchDelete">删除</a-button>
        <a-dropdown v-hasPermission="'admin:dict:export'">
          <a-menu slot="overlay">
      ...
      <a-table ref="TableInfo"
        ...
        <template slot="operation" slot-scope="text, record">
          <a-icon v-hasPermission="'admin:dict:update'" type="setting" theme="twoTone" twoToneColor="#4a9ff5" @click="edit(record)" title="修改字典"></a-icon>
          <a-badge v-hasNoPermission="'admin:dict:update'" status="warning" text="无权限"></a-badge>
        </template>
      </a-table>
    </div>
    <!-- 新增字典 -->
  ...
  methods: {
    ...
    batchDelete () {
      ...
      this.$confirm({
        ...
        centered: true,
        onOk () {
          that.$delete('admin/dict/' + that.selectedRowKeys.join(',')).then(() => {
