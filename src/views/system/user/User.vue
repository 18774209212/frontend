<template>
  <a-card :bordered="false"
    ...
    <div>
      <div class="operator">
        <a-button type="primary"
                  ghost
                  @click="add"
                  v-hasPermission="'admin:user:add'">新增</a-button>
        <a-button @click="batchDelete"
                  v-hasPermission="'admin:user:delete'">删除</a-button>
        <a-dropdown v-hasAnyPermission="'admin:user:reset'">
          <a-menu slot="overlay">
            <a-menu-item v-hasPermission="'admin:user:reset'"
                         key="password-reset"
                         @click="resetPassword">密码重置</a-menu-item>
            <a-menu-item v-hasPermission="'admin:user:export'"
                         key="export-data"
      ...
      <a-table ref="TableInfo"
        ...
        <template slot="operation"
                  slot-scope="text, record">
          <a-icon v-hasPermission="'admin:user:update'"
                  type="setting"
                  theme="twoTone"
                  twoToneColor="#4a9ff5"
                  @click="edit(record)"
                  title="修改用户"></a-icon>
          &nbsp;
          <a-icon v-hasPermission="'admin:user:view'"
                  type="eye"
                  theme="twoTone"
                  twoToneColor="#42b983"
                  @click="view(record)"
                  title="查看"></a-icon>
          <a-badge v-hasNoPermission="'admin:user:update','admin:user:view'"
                   status="warning"
                   text="无权限"></a-badge>
        </template>
      </a-table>
    ...
    <!-- 用户信息查看 -->
    <user-info :userInfoData="userInfo.data"
               :userInfoVisiable="userInfo.visiable"
               @close="handleUserInfoClose">
    </user-info>
    <!-- 新增用户 -->
    <user-add @close="handleUserAddClose"
...
export default {
  ...
  methods: {
    ...
    handleUserAddSuccess () {
      this.userAdd.visiable = false
      this.$message.success('新增用户成功，初始密码为abc1234')
      this.search()
    },
    edit (record) {
    ...
    resetPassword () {
      ...
      this.$confirm({
        title: '确定重置选中用户密码?',
        content: '当您点击确定按钮后，这些用户的密码将会重置为abc1234',
        centered: true,
        onOk () {
          let usernames = []
          let r
          for (let i = 0; i < that.dataSource.length; i++) {
            r = that.dataSource[i]
            if (that.selectedRowKeys.indexOf(r.userId) !== -1) {
