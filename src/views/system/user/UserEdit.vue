    ...
    <a-form :form="form">
      ...
      <a-form-item label='角色'
        ...
        <a-select mode="multiple"
          ...
          <a-select-option v-for="r in roleData"
                           :key="r.roleId.toString()">{{r.roleName}}</a-select-option>
        </a-select>
      </a-form-item>
      <a-form-item label='企业'
        ...
        <a-select :allowClear="true"
          ...
          <a-select-option v-for="r in companyData"
                           :key="r.companyId">{{r.companyName}}</a-select-option>
        </a-select>
      </a-form-item>
...
export default {
  ...
  methods: {
    ...
    setFormValues ({ ...user }) {
      ...
      let fields = ['username', 'email', 'status', 'ssex', 'mobile', 'companyId']
    ...
    handleSubmit () {
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          this.$put('admin/user', values).then((r) => {
            this.loading = false
            this.$emit('success')
            // 如果修改用户就是当前登录用户的话，更新其state
            if (values.username === this.currentUser.username) {
              this.$get(`user/${values.username}`).then((r) => {
                this.setUser(r.data)
              })
            }
          }).catch(() => {
  ...
  watch: {
    userEditVisiable () {
      if (this.userEditVisiable) {
        ...
        this.$get('admin/company').then((r) => {
          this.companyData = r.data.rows
        })
      }
    }
  }
