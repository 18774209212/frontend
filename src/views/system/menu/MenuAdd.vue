  ...
  <a-drawer title="新增菜单"
    ...
    <a-form :form="form">
      ...
      <a-form-item label='菜单排序'
                   v-bind="formItemLayout">
        <a-input-number v-decorator="['orderNum', {initialValue:''}]"
                        style="width: 100%" />
      </a-form-item>
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          if (checkedArr.length) {
            values.parentId = checkedArr[0]
          } else {
            values.parentId = ''
          }
          // 0 表示菜单 1 表示按钮
          values.type = '0'
          this.$post('admin/menu', values).then(() => {
            this.reset()
            this.$emit('success')
          }).catch(() => {
            this.loading = false
